...
ATower::ATower()
{
	...
	TriggerCapsuleInternal->SetCollisionProfileName(TEXT("OverlapAll"));
	...
	TriggerCapsuleExternal->SetCollisionProfileName(TEXT("OverlapAll"));
	...
	CollisionEnergy->SetCollisionProfileName(TEXT("Custom"));
	...
	NiagaraNet->SetComponentTickInterval(0.04f);
...
void ATower::MainFinder(ATower* TTower)
{
	...
	GetWorldTimerManager().SetTimer(TimerHandle0, [this, TTower]()
	{*/
			...
			{
				TTower->NetOff();
				TTower->GetWorldTimerManager().ClearAllTimersForObject(TTower);
				TTower->Destroy();
			}
...
void ATower::PartCommandFunc(bool Up)
{
	...
	{
		FTimerHandle TimerHandle;
		GetWorldTimerManager().SetTimer(TimerHandle, [this, Up]()
			{
